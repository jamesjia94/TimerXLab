...
-(void)goToTimer2{
    ...
    if (_uniformSwitch.on == YES)
    {
        UIStoryboard *timerUniformStoryboard= [UIStoryboard storyboardWithName:@"TimerUniformStoryboard" bundle:nil];
        UIViewController *timerUniformVC =[timerUniformStoryboard instantiateInitialViewController];
        timerUniformVC.modalTransitionStyle = UIModalTransitionStyleFlipHorizontal;
        [self presentViewController:timerUniformVC animated:YES completion:nil];
    }
    else
    {
